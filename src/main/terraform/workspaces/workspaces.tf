data "template_file" "workspaces_stack_file" {
  template      = "${file("${path.module}/workspaces.yml")}"
}

resource "aws_cloudformation_stack" "workspaces_stack" {
  name = "workspaces-stack"
  parameters    = {
    vpcId             = "${var.aws_vpc_id}"
    subnetId1         = "${var.aws_subnet_id1}"
    subnetId2         = "${var.aws_subnet_id2}"
    directoryName     = "${var.aws_directory_name}"
    directoryPassword = "${var.aws_directory_password}"
  }
  template_body = "${data.template_file.workspaces_stack_file.rendered}"
}
